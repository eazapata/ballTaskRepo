...
public class Ball implements Runnable, VisualObject, Serializable {
    ...
    public void moveBall(String action) {
        ...
        if (!this.stopped) {
            if (action.equals("right") && (this.channel.isOk()) && this.ballTask.getDirection().equals("derecha")) {
                this.running = false;
                System.out.println("enviando pelota " + this.channel.isOk());
                this.channel.send(this);
            } else if (action.equals("left") && this.channel.isOk() && this.ballTask.getDirection().equals("izquierda")) {
                 this.running = false;
                System.out.println("enviando pelota " + this.channel.isOk());
                this.channel.send(this);
